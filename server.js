...
//load place
app.get('/init', authenticateUser,function(req,res){
	Loc.findOne({id:"H1"},function(err,loc){
		if(!loc){
				var l = new Loc ({id: "H1", name: "Gloucester Road eastbound near the Revenue Tower", latitude: 22.279311622, longitude: 114.172101664});
				l.save();
		}
	});
	Loc.findOne({id:"H2"},function(err,loc){
		if(!loc){
				var l = new Loc ({id:"H2", name:"Canal Road Flyover northbound near exit of Aberdeen Tunnel", latitude: 22.271587756, longitude: 114.180185283});
				l.save();
		}
	});
	Loc.findOne({id:"H3"},function(err,loc){
		if(!loc){
				var l = new Loc ({id:"H3", name:"Island Eastern Corridor westbound near City Garden", latitude: 22.292018819, longitude: 114.193869616});
				l.save();
		}
	});
	Loc.findOne({id:"H11"},function(err,loc){
		if(!loc){
				var l = new Loc ({id:"H11", name:"Island Eastern Corridor westbound near Lei King Wan", latitude: 22.285235888, longitude: 114.221346549});
				l.save();
		}
	});
	Loc.findOne({id:"K01"},function(err,loc){
		if(!loc){
				var l = new Loc ({id:"K01", name:"Ferry Street southbound near Charming Gardena", latitude: 22.315044916, longitude: 114.166786346});
				l.save();
		}
	});
	Loc.findOne({id:"K02"},function(err,loc){
		if(!loc){
				var l = new Loc ({id:"K02", name:"Gascoigne Road eastbound near the Hong Kong Polytechnic University", latitude: 22.305896494, longitude: 114.178474746});
				l.save();
		}
	});
	Loc.findOne({id:"K03"},function(err,loc){
		if(!loc){
				var l = new Loc ({id:"K03", name:"Waterloo Road southbound near Kowloon Hospital", latitude: 22.324182195, longitude: 114.178378266});
				l.save();
		}
	});
	Loc.findOne({id:"K04"},function(err,loc){
		if(!loc){
				var l = new Loc ({id:"K04", name:"Princess Margaret Road southbound near Oi Man Estate", latitude: 22.31331063, longitude: 114.176931903});
				l.save();
		}
	});
	Loc.findOne({id:"K05"},function(err,loc){
		if(!loc){
				var l = new Loc ({id:"K05", name:"Kai Fuk Road northbound near the petrol stations", latitude: 22.319902728, longitude: 114.206732056});
				l.save();
		}
	});
	Loc.findOne({id:"K06"},function(err,loc){
		if(!loc){
				var l = new Loc ({id:"K06", name:"Chatham Road North southbound near Fat Kwong Street Playground", latitude: 22.309177485, longitude: 114.184084134});
				l.save();
		}
	});
	Loc.findOne({id:"SJ1"},function(err,loc){
		if(!loc){
				var l = new Loc ({id:"SJ1", name:"Tai Po Road – Sha Tin near the Racecourse", latitude: 22.404797024, longitude: 114.20734796});
				l.save();
		}
	});
	Loc.findOne({id:"SJ2"},function(err,loc){
		if(!loc){
				var l = new Loc ({id:"SJ2", name:"Tate’s Cairn Highway near Shek Mun", latitude: 22.389757785, longitude: 114.210593926});
				l.save();
		}
	});
	Loc.findOne({id:"SJ3"},function(err,loc){
		if(!loc){
				var l = new Loc ({id:"SJ3", name:"Tolo Highway near Science Park", latitude: 22.426975789, longitude: 114.206972621});
				l.save();
		}
	});
	Loc.findOne({id:"SJ4"},function(err,loc){
		if(!loc){
				var l = new Loc ({id:"SJ4", name:"San Tin Highway near Pok Wai Road", latitude: 22.464106829, longitude: 114.053710459});
				l.save();
		}
	});
	Loc.findOne({id:"SJ5"},function(err,loc){
		if(!loc){
				var l = new Loc ({id:"SJ5", name:"Tuen Mun Road near Tuen Mun Heung Sze Wui Road", latitude: 22.401075772, longitude: 113.97722659});
				l.save();
		}
	});
	res.send("done");
